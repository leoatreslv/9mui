...
# Chinese: "提醒我", "帮我提醒", "提醒一下"
_REMIND_RE = re.compile(
    r"(?:,|\s)*"
    r"(?:remind(?:er)?\s+me|提醒我|帮我提醒|提醒一下)"
    r"\s*(?:at|in|on|by|to|tomorrow|next)?\s*"  # English prepositions only — keep Chinese time words in suffix
    r"(.+)",
    re.IGNORECASE | re.DOTALL,
)

# English: "... at 4pm" / "... by 3pm" at end of message
# Chinese: "... 下午4点" / "... 明天9点" at end of message
_AT_TIME_RE = re.compile(
    r"(?:,|\s)*\b(?:at|by)\s+(\d[\d:]*\s*[apmAPM]{2}|\d{1,2}:\d{2})\s*$",
    re.IGNORECASE,
)

# Chinese time at end of message: "明天下午4点", "5分钟后", "下午3点"
_ZH_TIME_RE = re.compile(
    r"((?:今天|明天|后天)?(?:早上|上午|下午|晚上|夜)?(?:\d{1,2})[点时](?:\d{1,2}分)?"
    r"|(?:\d+)(?:分钟?|小时)后)"
    r"\s*$"
)

# Trigger phrases that indicate content precedes the time in Chinese
_ZH_TRIGGER_RE = re.compile(r"提醒我|帮我提醒|提醒一下")


_ZH_DAY_MAP = {"今天": 0, "明天": 1, "后天": 2}
...
def _parse_zh_clock(text: str) -> Optional[datetime]:
    """Parse Chinese clock expressions like 明天下午4点, 早上9点半, 下午3点30分."""
    ...
    # If no day specified and time already passed today, assume tomorrow
...
_TIME_WORD_RE = re.compile(
    ...
    r"点|时|分钟|小时|明天|后天|今天|下午|上午|晚上|早上|夜",
    re.IGNORECASE,
)
...
def parse_message(text: str, timezone: str = "UTC") -> Tuple[str, Optional[datetime]]:
    ...
    if m:
        ...
        if remind_at and not content:
            # "X分钟后content" or "X小时后content" — content follows 后
            dur_m = re.search(r"\d+(?:分钟?|小时)后", suffix)
            if dur_m:
        ...
        if not remind_at and has_content_before_time:
            ...
            if remind_at and not content:
                ...
                if results:
                    ...
                    content = re.sub(re.escape(time_str) + r"后?", "", suffix, flags=re.IGNORECASE)
                    content = re.sub(r"^(?:to\s+|去|做|帮)", "", content, flags=re.IGNORECASE)
    ...
    # ── Case 2: Chinese time at end — "叫老婆买菜 明天下午3点" ──────────────
